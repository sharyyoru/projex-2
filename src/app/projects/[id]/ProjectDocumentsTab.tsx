...
export default function ProjectDocumentsTab({
  ...
  useEffect(() => {
    ...
    async function loadItems() {
      ...
      const folderPath = [base, currentPrefix].filter(Boolean).join("/");

      let listPath: string | undefined = folderPath === "" ? undefined : folderPath;
      let usedFallback = false;

      let data: any[] | null = null;
      let listError: any = null;

      const primaryResult = await supabaseClient.storage
        .from(BUCKET_NAME)
        .list(listPath, {
          limit: 200,
          offset: 0,
      ...
      listError = primaryResult.error ?? null;
      ...
      if (listError) {
        setError(listError.message ?? "Failed to load files.");
        setItems([]);
      ...
      const pathLabel = listPath ?? "(root)";
      const debugLine = `ProjectDocumentsTab: listed path ${JSON.stringify(
        pathLabel,
      )} (base=${JSON.stringify(
        base,
      )}, fallback=${usedFallback}) → raw items: ${rawCount}, visible items: ${
        combined.length
      }`;
      setDebugInfo(debugLine);
      // Lightweight runtime debug to help align storage paths in non-production usage.
  ...
  }, [projectId, basePrefix, currentPrefix, selectedFile, refreshKey]);
  ...
  const selectedFilePreviewUrl = useMemo(() => {
    ...
    const base = (basePrefix ?? projectId) || "";
    const fullPath = [base, selectedFile.path].filter(Boolean).join("/");
  ...
  }, [projectId, basePrefix, selectedFile]);
        ...
        try {
          ...
          const body =
            uploadedFileNames.length === 1
              ? `Uploaded file "${uploadedFileNames[0]}" to the project files.`
              : `Uploaded ${uploadedFileNames.length} files to the project files: ${uploadedFileNames
                  .map((name) => `"${name}"`)
                  .join(", ")}.`;
          ...
          if (insertError) {
            // eslint-disable-next-line no-console
            console.error(
              "ProjectDocumentsTab: failed to insert upload activity note",
              insertError,
            );
          } else if (onActivityEvent) {
            onActivityEvent();
          }
        } catch (err) {
          // eslint-disable-next-line no-console
          console.error(
            "ProjectDocumentsTab: unexpected error creating upload activity note",
            err,
          );
        }
      }
  ...
  async function handleCreateFolder(event: FormEvent) {
    ...
    try {
      ...
      const base = (basePrefix ?? projectId) || "";
      const fullPath = [base, folderPath, ".keep"].filter(Boolean).join("/");
      ...
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-xs text-slate-500">
            Store, organise, and preview files for this project.
          </p>
        </div>
          ...
          <div className="max-h-80 overflow-auto rounded-lg border border-slate-100 bg-slate-50/60 p-2">
            {items.length === 0 ? (
              <div className="flex h-24 items-center justify-center text-[11px] text-slate-500">
                No files yet. Use the Upload button to add files.
              </div>
            ) : (
